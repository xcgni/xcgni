...
credentials - no SSH keys, no VPN material. CI publishes; the server pulls.

Pipeline: on every green version tag (needs test + build + integration), a publish job
pushes ghcr.io/xcgni/xcgni:{vX.Y.Z, latest} using only the built-in GITHUB_TOKEN. A
smoke job then boots THAT exact image against a live Postgres and hits it from outside:
/api/health (new endpoint: liveness + DB ping), the homepage, /statistics, and a
cookie-jar POST to /api/practice/next asserting no 5xx - the runtime-wiring class that
cost three local builds this week now dies in Actions. Also added: a version-consistency
gate (VERSION must equal package.json), svelte-check as an advisory step, lockfile-aware
installs everywhere (npm ci once package-lock.json lands - one maintainer command,
noted below), and a concurrency group that cancels superseded runs.

Server side: docker-compose.prod.yml switches the app from build: to
image: ghcr.io/xcgni/xcgni with IMAGE_TAG pin (rollback = one .env line).
scripts/deploy-pull.sh compares image digests and recreates only on change, refuses on
invalid compose config, prunes old images, logs every decision; deploy/ ships the
systemd oneshot + 5-minute timer and a README with the ten-minute setup. Preferred
credential story: make the GHCR package public and there is no credential at all.

Maintainer TODO with this release: run npm install --package-lock-only and commit
package-lock.json (the build environment here has no registry access); make the GHCR
package public after the first publish; then follow deploy/README-deploy.md once. The
zip-and-rsync era ends with the next tag.
...
## v1.13.0 - Three languages: the interface learns Croatian and German
